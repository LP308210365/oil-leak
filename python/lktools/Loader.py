...
#TODO(刘鹏)：处理path为url的情况, 添加对webcams.json文件的读取
def get_settings():
  ...
  #TODO(刘鹏）：添加webcams.json文件的检查
  if not os.path.exists('settings.json') or \
     not os.path.exists('resources/data/template.json') or \
    not os.path.exists('webcams.json'):
